...
export default function AnemiaScreening() {
  ...
  const [result, setResult] = useState<"HIGH" | "LOW" | "UNABLE TO ASSESS" | null>(null);
  ...
  const [scanType, setScanType] = useState<"eye" | "hand">("eye");
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      handleCapture(file);
      submitToApi([file]); // Analyze immediately
    }
  };

  const submitToApi = async (overrideCaptures?: Blob[]) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const { error } = await supabase.from('screenings').insert({
        ...
        result_label: result === "HIGH" ? "Elevated Screening Risk" : "Lower Screening Risk",
        risk_level: result === "HIGH" ? "elevated" : "low",
        confidence: probability,
        model_version: 'anemia_v3_eye_multitask'
      });
      ...
      if (result === "HIGH") {
        await supabase.from('followups').insert({
          patient_id: user.id,
          reason: 'Elevated Anaemia Screening Risk - Haemoglobin Test Recommended',
          priority: 'urgent',
          status: 'pending'
        });
      }
  ...
  const getTranslatedResult = (res: string | null) => {
    if (res === "HIGH") return (t as any).results?.high || "High Risk";
    if (res === "LOW") return (t as any).results?.low || "Low Risk";
    ...
    return (
      <main className="mx-auto min-h-screen max-w-md bg-[#f8faf9] p-6 pb-28 flex flex-col items-center justify-center font-sans">
        ...
        <div className="flex items-center gap-3 mb-4">
          <h1 className="text-2xl font-black text-gray-800 text-center">{t.screenings.anaemia}</h1>
      ...
      <div className="bg-black min-h-screen">
        <div className="absolute top-0 left-0 right-0 z-[60] bg-black bg-opacity-70 text-white p-4 flex flex-col items-center">
          <div className="text-lg font-bold mb-2">
            Capture {scanType === "eye" ? "palpebral conjunctiva" : "palm of your hand"}
          </div>
  ...
  if (mode === "result") {
    return (
      <main className="mx-auto min-h-screen max-w-md bg-[#f8faf9] p-6 pb-28 font-sans">
        ...
        <RiskMeter probability={probability} threshold={threshold} />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6 text-left space-y-3">
          ...
          <div className="flex justify-between items-center border-b border-gray-50 pb-2">
            <span className="text-sm font-semibold text-gray-600">{(t as any).ui?.estimatedHb || "Estimated Hb"}</span>
            <span className="text-sm font-bold text-gray-800">{hgb !== null ? `${hgb.toFixed(1)} g/dL` : 'N/A'}</span>
          </div>
          <div className="flex justify-between items-center border-b border-gray-50 pb-2">
            <span className="text-sm font-semibold text-gray-600">{(t as any).ui?.anemiaRisk || "Anemia Risk"}</span>
            <span className={`text-sm font-bold ${result === 'HIGH' ? 'text-red-500' : 'text-emerald-500'}`}>{getTranslatedResult(result)}</span>
          </div>
        ...
        <div className="text-center mt-2 mb-8">
          <p className="text-[10px] font-medium text-gray-400 italic">
            {(t as any).ui?.medicalDisclaimer || "Confirmation requires a standard hemoglobin test."}
          </p>
        </div>
